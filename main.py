...
# --- RAILWAY FIX (Életben tartó szerver) ---
app = Flask('')
@app.route('/')
...
class UltimateBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        print(f"✅ {self.user} Online! Használd a !sync parancsot a szerveren!")

bot = UltimateBot()

# --- SZINKRONIZÁLÁS (Kényszerítés, ha nem látod a parancsokat) ---
@bot.command()
async def sync(ctx):
    if ctx.author.guild_permissions.administrator:
        bot.tree.copy_global_to(guild=ctx.guild)
        synced = await bot.tree.sync(guild=ctx.guild)
        await ctx.send(f"✅ **{len(synced)}** slash parancs aktiválva ezen a szerveren!")
...
async def on_message(message):
    ...
    # AUTOMOD (Csak nem moderátoroknak)
...
# --- SLASH PARANCSOK (MODERÁTOROKNAK) ---
...
@bot.tree.command(name="munkaido", description="📊 Statisztika és szintek (500 / 10000 pont)")
@app_commands.checks.has_permissions(moderate_members=True)
async def munkaido(interaction: discord.Interaction, tag: discord.Member):
    ...
    # SZINTEK: 500 pont (Jr. Mod), 10000 pont (Mod)
