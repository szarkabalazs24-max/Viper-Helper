...
# --- KONFIGURÁCIÓ & ÚTVONAL (VOLUME-hoz) ---
TOKEN = os.environ.get('DISCORD_TOKEN')
DB_PATH = '/app/data/database.db' # Ez mentődik a Volume-ba
BAD_WORDS = ["geci", "buzi", "kurva", "anyád", "fasz", "szar", "köcsög"]
LINK_PATTERN = r"(https?://[^\s]+)"

class UltimateBot(commands.Bot):
    def __init__(self):
        # Minden intent bekapcsolva a teljes működéshez
        intents = discord.Intents.all()
        super().__init__(command_prefix="!", intents=intents)
        self.antispam = {}

    async def setup_hook(self):
        # ADATBÁZIS INICIALIZÁLÁSA A VOLUME-BAN
        ...
        # GLOBÁLIS SZINKRONIZÁLÁS (Discord API-nak küldés)
        # Ez regisztrálja a / parancsokat a Discord rendszerében
        await self.tree.sync()
        print(f"✅ {self.user} Online! Slash parancsok regisztrálva.")
        print("💡 Tipp: Ha nem látod a parancsokat, használd a !sync parancsot a szerveren!")

bot = UltimateBot()

# --- SZINKRONIZÁLÁS (Kényszerített frissítés a szerverre) ---
@bot.command()
async def sync(ctx):
    """Kézzel kényszeríti a slash parancsok megjelenését az adott szerveren"""
    if ctx.author.guild_permissions.administrator:
        try:
            # Átmásoljuk a globális parancsfát a jelenlegi szerverre
            bot.tree.copy_global_to(guild=ctx.guild)
            synced = await bot.tree.sync(guild=ctx.guild)
            await ctx.send(f"✨ **{len(synced)}** slash parancs kényszerítve és frissítve ezen a szerveren!")
        except Exception as e:
            await ctx.send(f"❌ Hiba a szinkronizálás során: {e}")
...
async def on_message(message):
    ...
    if not message.author.guild_permissions.manage_messages:
        ...
        violation = None
        v_emoji, v_color = "🛡️", 0xFFAA00

        if any(w in content for w in BAD_WORDS): violation, v_emoji, v_color = "Káromkodás", "🤬", 0xE74C3C
        elif re.search(LINK_PATTERN, content): violation, v_emoji, v_color = "Tiltott link", "🔗", 0x3498DB
        ...
        if len(bot.antispam[uid_int]) > 5: violation, v_emoji, v_color = "Spamming", "🚀", 0x9B59B6
        ...
        if violation:
            try:
                ...
                embed.set_footer(text="Ez az üzenet 5 másodperc múlva törlődik.")
                await message.channel.send(embed=embed, delete_after=5)
                return # Ha szabályszegés történt, itt megállunk, nem kap pontot
            except:
                pass

    # MUNKAIDŐ (15 üzenet = 1 pont)
    ...
    # Fontos: Ez engedi futni a hagyományos (prefix-es) parancsokat, mint a !sync
...
@bot.tree.command(name="figyelmeztetes", description="⚠️ Tag figyelmeztetése és automatikus némítása")
...
async def warn(interaction: discord.Interaction, tag: discord.Member, indok: str):
    if tag.top_role >= interaction.user.top_role:
        return await interaction.response.send_message("❌ Nem büntethetsz veled egyenrangú vagy feletted álló tagot!", ephemeral=True)
    ...
    # SZANKCIÓ TÁBLÁZAT (1-3: 10p, 4-5: 1ó, 6+: 1nap)
    ...
    try: 
        await tag.send(f"⚠️ Figyelmeztetést kaptál a(z) **{interaction.guild.name}** szerveren!", embed=embed)
    except: 
        pass

@bot.tree.command(name="munkaido", description="📊 Moderátori statisztika és szintek")
@app_commands.checks.has_permissions(moderate_members=True)
async def munkaido(interaction: discord.Interaction, tag: discord.Member):
    ...
    # SZINTEK: 1. Jr. Mod (500), 2. Mod (10000)
    ...
    embed = discord.Embed(title="📊 Munkaidő Adatlap", color=0x3498DB, timestamp=discord.utils.utcnow())
    embed.set_thumbnail(url=tag.display_avatar.url)
...
@app_commands.checks.has_permissions(moderate_members=True)
async def delwarn(interaction: discord.Interaction, tag: discord.Member, warn_id: str, indok: str):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT wid FROM history WHERE wid = ? AND uid = ?", (warn_id, str(tag.id)))
    if not c.fetchone():
        conn.close()
        return await interaction.response.send_message("❌ **Hiba:** Érvénytelen ID!", ephemeral=True)
    ...
    embed.add_field(name="📝 Indok", value=f"*{indok}*", inline=False)
...
async def clear(interaction: discord.Interaction, mennyiseg: int):
    # A halasztott válasz (defer) segít, ha a törlés több mint 3 másodpercig tart
    await interaction.response.defer(ephemeral=True)
    deleted = await interaction.channel.purge(limit=mennyiseg)
    await interaction.followup.send(f"✅ Sikeresen törölve: **{len(deleted)}** üzenet.")

# --- INDÍTÁS ---
if __name__ == "__main__":
    # Flask szál indítása Railway életben tartáshoz
