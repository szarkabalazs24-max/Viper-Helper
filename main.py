...
async def sync(ctx):
    if ctx.author.guild_permissions.administrator:
        ...
        await ctx.send(f"✅ **{len(synced)}** slash parancs sikeresen aktiválva ezen a szerveren!")
...
@bot.tree.command(name="figyelmeztetes", description="⚠️ Tag warnolása és némítása")
...
@bot.tree.command(name="munkaido", description="📊 Statisztika és szintek")
@app_commands.checks.has_permissions(moderate_members=True)
async def munkaido(interaction: discord.Interaction, tag: discord.Member):
    ...
    embed = discord.Embed(title="📊 Munkaidő Adatlap", color=0x3498DB)
    embed.set_thumbnail(url=tag.display_avatar.url)
...
async def delwarn(interaction: discord.Interaction, tag: discord.Member, warn_id: str, indok: str):
    ...
    if not c.fetchone():
        conn.close()
        return await interaction.response.send_message("❌ Érvénytelen ID!", ephemeral=True)
    ...
    embed.add_field(name="Törlés oka", value=indok)
    await interaction.response.send_message(embed=embed)
