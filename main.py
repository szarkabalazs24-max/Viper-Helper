...
# --- RAILWAY ÉLETBEN TARTÁS (FLASK) ---
app = Flask('')
@app.route('/')
def home(): return "Bot is Online!", 200

def run_web():
    # Railway automatikusan ad portot, ha nincs, a 8080-at használja
...
class UltimateBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        # Időzített feladatok indítása
        if not self.voice_tracker.is_running():
            self.voice_tracker.start()
        
        # Automatikus szinkronizálás az indításkor
    ...
    async def voice_tracker(self):
        try:
            ...
            print(f"Voice tracker hiba: {e}")

bot = UltimateBot()

# --- MANUÁLIS SZINKRONIZÁLÁS (EMBED) ---
@bot.command()
async def sync(ctx):
...
async def warn(interaction: discord.Interaction, tag: discord.Member, indok: str):
    if tag.top_role >= interaction.user.top_role:
        return await interaction.response.send_message("❌ Hiba: Te nem büntetheted őt!", ephemeral=True)
...
if __name__ == "__main__":
    if TOKEN:
        # Flask szál indítása (hogy ne blokkolja a botot)
        ...
        print("❌ HIÁNYZIK A DISCORD_TOKEN! Állítsd be a Railway Variables fülön!")
