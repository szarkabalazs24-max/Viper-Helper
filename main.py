...
class UltimateBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        print(f"✅ {self.user} Online! Használd a !sync parancsot!")

bot = UltimateBot()

# --- SZINKRONIZÁLÁS (HOGY LÁSD A PARANCSOKAT) ---
@bot.command()
async def sync(ctx):
    if ctx.author.guild_permissions.administrator:
        bot.tree.copy_global_to(guild=ctx.guild)
        synced = await bot.tree.sync(guild=ctx.guild)
        await ctx.send(f"✨ **{len(synced)}** slash parancs kényszerítve a szerverre!")
...
async def warn(interaction: discord.Interaction, tag: discord.Member, indok: str):
    ...
    try: await tag.send(f"⚠️ Figyelmeztetést kaptál a(z) **{interaction.guild.name}** szerveren!", embed=embed)
    except: pass
